...
为了解决上述问题，C99 标准引入了变长数组，它允许使用变量定义数组的各个维度，但变长数组有一些限制：变长数组必须是自动存储类的，也就是说他们必须在函数内部或作为函数的参量声明，而且声明时不可以进行初始化。
...
隐式转换的风险一般存在与自定义的类构造方法中，按照默认规则，**只有一个参数的构造函数**定义了一个隐式转换，将该构造函数对应数据类型的数据转换为该类对象
