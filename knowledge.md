...
- 若客户发出的SYN在中间的某个路由上引发了一个“destination unreachable”（目的不可达）ICMP错误，客户主机内核保存该消息，并按1中所述的时间间隔发送SYN，在某个规定的时间（4.4BSD规定75s）仍未收到响应，则把保存的ICMP错误作为**EHOSTUNREACH或ENETUNREACH错误**返回给进程。
...
- Bob计算要传输文件的Hash值，将Hash值用 Bob 的私钥加密，得到摘要，将**摘要，Bob的证书和文件**一同发送给 Alice
