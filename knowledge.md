...
在实际的场景中，每个酒店房间都会安装一个智能音箱，音箱是交互的方式之一，客户对房间内的音箱说“打开窗帘、我要看直播”这样的语句，房间内的窗帘会自动打开，电视会打开开始播放，这些控制功能的调用链路是：当客户对音箱说完话，音箱通过一个开放平台经过语音识别解析为对应的请求消息，发送到我们的云住平台，云住平台通过音箱序列号查找该音箱是哪个集团的哪个分店的哪个房间，构造包含requestId、设备Id、属性值的gRPC请求消息发送到该房间的网关设备，网关设备解析请求消息对具体家具如窗帘、电视下发指令，实现对应的控制意图

web页面主要是给酒店管理人员使用，酒店管理人员可以在web页面上添加房间内的设备，比如音箱、窗帘、电视等等；管理房间模式，比如查房模式、睡眠模式；设置该分店的问答知识库等

举例自己从前端到后端开发的完整功能问答知识库：需求背景：客户可以通过与音箱对话，询问如：酒店早餐时间、附近的地铁站、景点等，所有酒店集团的问题是一样的，但这些问题的答案是与分店本身相关的，所以下放到云住平台实现更合理，酒店管理人员可以在web上对问题的答案进行修改，客户只需直接向音箱提问，音箱便可回复对应的答案

实现过程：1. 数据库中两张表：一张问题表、一张问题酒店关联表；问题表主要包括问题Id、问题内容、创建修改时间等，所有分店的问题均相同，不同的是问题答案；问题酒店关联表主要包括问题Id，分店Id以及答案 2. proto文件定义两个后端接口，一个描述问答知识库的接口，用于前端展示当前分店问答知识库内容，一个更新问答知识库的接口，用于更新答案
