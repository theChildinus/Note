<!-- TOC -->autoauto- [简历相关](#简历相关)auto    - [项目1](#项目1)auto        - [Serviceability Agent](#serviceability-agent)auto    - [项目2](#项目2)auto    - [项目3](#项目3)auto- [数据结构](#数据结构)auto    - [红黑树](#红黑树)auto    - [B/B+树](#bb树)auto    - [epool libevent (异步I/O)](#epool-libevent-异步io)auto    - [布隆过滤器](#布隆过滤器)auto    - [字典树 AC自动机 fail节点 KMP](#字典树-ac自动机-fail节点-kmp)auto- [数据库](#数据库)auto    - [数据库存储过程、触发器](#数据库存储过程触发器)auto    - [数据库范式](#数据库范式)auto    - [ACID](#acid)auto    - [并发一致性问题](#并发一致性问题)auto    - [封锁](#封锁)auto    - [隔离级别](#隔离级别)auto    - [多版本并发控制](#多版本并发控制)auto    - [数据库锁机制](#数据库锁机制)auto        - [SQL语句锁实例分析](#sql语句锁实例分析)auto            - [提交读 RC 隔离级别下](#提交读-rc-隔离级别下)auto            - [可重复读 RR 隔离级别下](#可重复读-rr-隔离级别下)auto            - [串行化 Serializable 隔离级别下](#串行化-serializable-隔离级别下)auto    - [存储引擎](#存储引擎)auto        - [InnoDB](#innodb)auto        - [MyISAM](#myisam)auto    - [数据库索引](#数据库索引)auto        - [数据库索引分类](#数据库索引分类)auto            - [B+ Tree 索引](#b-tree-索引)auto            - [哈希索引](#哈希索引)auto            - [全文索引](#全文索引)auto        - [索引失效、优缺点、优化](#索引失效优缺点优化)auto        - [索引实现](#索引实现)auto        - [聚簇索引和非聚簇索引](#聚簇索引和非聚簇索引)auto    - [切分](#切分)auto        - [水平切分](#水平切分)auto        - [垂直切分](#垂直切分)auto    - [视图](#视图)auto    - [Redis](#redis)auto        - [数据结构和对象](#数据结构和对象)auto            - [简单动态字符串](#简单动态字符串)auto            - [链表](#链表)auto            - [字典](#字典)auto            - [跳跃表](#跳跃表)auto            - [整数集合](#整数集合)auto            - [压缩列表](#压缩列表)auto            - [对象](#对象)auto- [Linux](#linux)auto    - [I/O 模式](#io-模式)auto        - [阻塞I/O](#阻塞io)auto        - [非阻塞I/O](#非阻塞io)auto        - [I/O复用](#io复用)auto        - [信号驱动I/O](#信号驱动io)auto        - [异步I/O](#异步io)auto        - [对比](#对比)auto    - [内存管理](#内存管理)auto        - [Buddy分配算法](#buddy分配算法)auto        - [吞吐和响应](#吞吐和响应)auto        - [硬链接和软链接](#硬链接和软链接)auto- [其他](#其他)auto    - [SIP协议](#sip协议)auto    - [Web请求](#web请求)auto    - [HTTP协议](#http协议)auto        - [GET 和 POST 区别](#get-和-post-区别)auto        - [Cookie 和 Session](#cookie-和-session)auto    - [数字认证](#数字认证)auto    - [路由分组转发](#路由分组转发)auto    - [Reactor模式](#reactor模式)autoauto<!-- /TOC -->
...
chunked 传输不能事先知道内容长度，只能靠最后的空 chunked 来判断，因此对于下载请求来说无法实现进度，**chunked 的优势在于，服务器端可以边生成内容边发送，无需事先生成全部的内容。**HTTP/2 不支持 Transfer-Encoding: chunked，因为 HTTP/2 有自己的 streaming 传输方式
...
Token：客户端传送用户名和密码到服务器请求登陆，服务器在验证通过后签发一个有时效性的token，将token返回给客户端
